...
for ((i=0; i<$CASSANDRA_TABLES_LENGTH; i++)); do
    ...
    eval "$DSBULK_CMD load -h $CASSANDRA_HOST -port $CASSANDRA_PORT -k $CASSANDRA_KEYSPACE -t $TABLE_NAME -url $TABLE_FOLDER -header true -maxConcurrentQueries 10 -cl ONE --log.checkpoint.enabled false --connector.csv.maxCharsPerColumn -1"
done
